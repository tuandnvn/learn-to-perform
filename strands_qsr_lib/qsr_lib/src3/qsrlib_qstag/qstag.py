...
class Activity_Graph:
    ...
    def get_objects_types(objects_types, world):
        ...
        **kwargs value [object_type] in the World Trace object
        ...
        :rtype: dict
        """
        for t, state in list(world.trace.items()):
            for oname, odata in list(state.objects.items()):
...
def get_graphlet_selections(episodes, params, object_types, vis=False):
    ...
    hashed_IDs = {}
    range_of_rows = list(range(params["min_rows"], params["max_rows"]+1))
    for r in range_of_rows:
        # Once we select the number of rows, find all combinations of rows of r.
        for obj_pair_comb in combinations(list(intervals.keys()), r):
    ...
    if vis: print("\nepisode keys:", list(hashed_IDs.keys()))
    ...
    for objs, IDs in list(hashed_IDs.items()):
        #episodes_selection[objs] = []

        for hash_, id_codes in list(IDs.items()):
