...
class QSR_Arg_Relations_Abstractclass(QSR_Dyadic_1t_Abstractclass, metaclass=abc.ABCMeta):
    ...
    def __populate_possible_relations_and_values(self):
        ...
        ret_relations = []
        ret_values = []
        sorted_by_v = sorted(list(self.qsr_relations_and_values.items()), key=self.value_sort_key)
    ...
    def __check_validity_of_qsr_relations_and_values(self, qsr_relations_and_values):
        ...
        for k, v in list(qsr_relations_and_values.items()):
