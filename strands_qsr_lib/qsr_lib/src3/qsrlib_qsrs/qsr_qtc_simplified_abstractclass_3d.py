...
class QSR_QTC_Simplified_3d(QSR_Dyadic_Abstractclass, metaclass=ABCMeta):
    """QTCS abstract class.
    ...
    def make_world_qsr_trace(self, world_trace, timestamps, qsr_params, req_params, **kwargs):
        ...
        for t, tp, tpp in zip(timestamps[2:], timestamps[1:], timestamps):
            ...
            qsrs_for = self._process_qsrs_for([list(world_state_previous_previous.objects.keys()),  
                                                list(world_state_previous.objects.keys()), 
                                                list(world_state_now.objects.keys())],
