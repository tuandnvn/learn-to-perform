...
def compute_episodes(world_qsr):
	...
	for frame in frames:
		for objs, qsrs in list(world_qsr.trace[frame].qsrs.items()):
			my_qsrs = {}
			#print("h", objs, qsrs.qsr)

			for qsr_key, qsr_val in list(qsrs.qsr.items()):
	...
	for objs, frame_tuples in list(obj_based_qsr_world.items()):
	...
	for ep in episodes:
		ignore_flag = 0
		for qsr, val in list(ep[1].items()):
...
def get_E_set(objects, spatial_data):
	...
	:rtype: tuple
	"""
	objects_ids = list(objects.values())
	start, end = {}, {}
	E_s, E_f = [], []
